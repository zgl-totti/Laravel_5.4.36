...
var ZYFILE = {
		...
		funGetFiles : function(e){  
           ...
           /* if (uArr.length > 3) {
               layer.msg("最多上传3张图片",{icon:2,time:1400});
            }else{
            ...
            if (uArr.length > 3) {
                ...
                $.each(uArr, function(k, v){
                    if(uArr){
                        ...
                        if(v.Right(5).toLowerCase()==".jpeg" || v.Right(4).toLowerCase()==".jpg" || v.Right(4).toLowerCase()==".png"){
                            // 获得当前选择的每一个文件   判断当前这一个文件是否存在于之前的文件当中
                            if($.inArray(v, lArr) < 0){  // 不存在
                                tmpFiles.push(self.uploadFile[k]);
                            }
                        }else{
                            layer.msg("图片类型错误",{icon:2,time:1400});
                        }
                });
            }
